...
首次启动且尚未完成本地配置时，应用会显示播放器初始化页面。请按以下步骤完成设置：

1. 可选：在 **播放器名称** 中输入当前播放器的名称。
2. 可选：在 **地点** 中输入播放器所在的位置。
3. 在 **内容管理系统网址** 中输入 CMS 服务地址，例如 `http://121.40.137.228:8080/demo`。
4. 点击 **保存并继续**。应用会保存配置、加载播放器设置并启动内容同步服务，然后进入播放界面。
5. 如果暂时不想配置，点击 **取消并退出**；桌面端会退出应用，Android/iOS 设备会关闭当前应用页面。

**组织代码**、**频道** 和 **设置分组** 仍会使用程序内置的默认值保存到播放器配置中，但不会显示在设置页面。

## 浮动菜单

在播放画面快速连续点击两次，可打开底部浮动菜单。菜单包含以下功能：

- **设置**：打开应用设置页面。
- **重新初始化**：重新打开播放器初始化页面。输入 CMS 地址后保存，应用会重新获取播放设置和内容类型设置文件，并返回播放界面；播放器名称和地点可以留空。
- **退出**：退出应用。
- **返回**：关闭浮动菜单。
